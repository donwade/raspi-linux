...
/**
 * struct imx911_reg - imx911 sensor register
...
/**
 * struct imx911_reg_list - imx911 sensor register list
...
/**
 * struct imx911_mode - imx911 sensor mode structure
 * @width: Frame width
 * @height: Frame height
...
/**
 * struct imx911 - imx911 sensor device structure
 ...
 *
 * Return: pointer to imx911 device
 */
static inline struct imx911 *to_imx911(struct v4l2_subdev *subdev)
{
...
/**
 * imx911_update_exp_gain() - Set updated exposure and gain
 * @imx911: pointer to imx911 device
...
/**
 * imx911_set_ctrl() - Set subdevice control
...
static int imx911_code_to_mbus_code(struct tc358743_state *imx911, u32 code)
{
	...
	for (i = 0; i < ARRAY_SIZE(imx911_mbus_codes); i++) {
		if (imx911_mbus_codes[i] == code)
			return imx911_mbus_codes[i];
	}

	return imx911_mbus_codes[0];
}

/**
 * imx911_enum_mbus_code() - Enumerate V4L2 sub-device mbus codes
 * @sd: pointer to imx911 V4L2 sub-device structure
...
{
	if (code->index >= ARRAY_SIZE(imx911_mbus_codes))
		return -EINVAL;

	code->code = imx911_mbus_codes[code->index];

	return 0;
}

/**
 * imx911_enum_frame_size() - Enumerate V4L2 sub-device frame sizes
 * @sd: pointer to imx911 V4L2 sub-device structure
 * @sd_state: V4L2 sub-device state
...
/**
 * imx911_fill_pad_format() - Fill subdevice pad format
...
/**
 * imx911_get_pad_format() - Get subdevice pad format
 * @sd: pointer to imx911 V4L2 sub-device structure
 * @sd_state: V4L2 sub-device state
...
/**
 * imx911_set_pad_format() - Set subdevice pad format
 * @sd: pointer to imx911 V4L2 sub-device structure
 * @sd_state: V4L2 sub-device state
 * @fmt: V4L2 sub-device format need to be set
 *
...
/**
 * imx911_init_pad_cfg() - Initialize sub-device pad configuration
 * @sd: pointer to imx911 V4L2 sub-device structure
 * @sd_state: V4L2 sub-device state
 *
 * Return: 0 if successful, error code otherwise.
...
/**
 * imx911_start_streaming() - Start sensor stream
 * @imx911: pointer to imx911 device
 *
 * Return: 0 if successful, error code otherwise.
 */
...
/**
 * imx911_stop_streaming() - Stop sensor stream
 * @imx911: pointer to imx911 device
 *
 * Return: 0 if successful, error code otherwise.
 */
...
/**
 * imx911_set_stream() - Enable sensor streaming
 * @sd: pointer to imx911 subdevice
...
/**
 * imx911_init_controls() - Initialize sensor subdevice controls
 * @imx911: pointer to imx911 device
 *
 * Return: 0 if successful, error code otherwise.
 */
...
/**
 * imx911_probe() - I2C client device binding
